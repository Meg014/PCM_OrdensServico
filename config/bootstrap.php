...
/*
 * CLI, web and scheduled tasks share this file, independently of their working
 * directory. File values override process variables to avoid different DB_*
 * values inherited by IIS/Apache and interactive shells. Without this file,
 * provision the same environment for every process. Never commit config/.env.
 */
if (file_exists(CONFIG . '.env')) {
    try {
        (new Loader([CONFIG . '.env']))
            ->parse()
            ->putenv(true)
            ->toEnv(true)
            ->toServer(true);
    } catch (Throwable) {
        // Parser errors can include secret values from the offending line.
        throw new RuntimeException('Não foi possível carregar config/.env; verifique formato e permissões.');
    }
}
