...
<section class="pcm-dashboard-section pcm-history-section" aria-labelledby="history-title">
    ...
    <?php else: ?>
        ...
        <div class="row g-3">
            <?php foreach ([['Novas O.S.', $movement['new'], '+', 'new'], ['Passaram para fechada', $movement['toCompleted'], '+', 'completed'], ['Passaram para em aberto', $movement['toOpen'], '+', 'open'], ['Não presentes no snapshot atual', $movement['absent'], '−', 'absent'], ['Passaram para cancelada', $movement['toCancelled'], '+', 'cancelled']] as [$label, $value, $symbol, $type]): ?>
                <div class="col-sm-6 col-xl"><article class="pcm-movement-card"><span><?= h($label) ?></span><strong><?= h($symbol) ?> <?= number_format((int)$value, 0, ',', '.') ?></strong><?= $this->Html->link('Ver O.S.', ['_name' => 'pcm-movement', 'type' => $type, '?' => isset($area) ? ['area' => $area->source_code] : []]) ?></article></div>
            <?php endforeach; ?>
        </div>
        <div class="row g-4 mt-1">
            ...
            <div class="col-12"><div class="pcm-chart-card"><h3>Equipamentos com mais ocorrências no período</h3><p>Quantidade de O.S. distintas observadas; a mesma O.S. não é somada a cada snapshot.</p><div class="pcm-chart-wrap"><canvas data-pcm-history="equipment"></canvas></div></div></div>
        </div>
        <details class="pcm-panel pcm-transition-panel mt-4"><summary>Transições de STATUS observadas</summary><div class="table-responsive"><table class="table mb-0"><thead><tr><th>STATUS anterior</th><th>STATUS atual</th><th>O.S.</th></tr></thead><tbody><?php foreach ($movement['transitions'] as $transition): ?><tr><td><?= h($transition['from']) ?></td><td><?= h($transition['to']) ?></td><td><?= number_format((int)$transition['quantity'], 0, ',', '.') ?></td></tr><?php endforeach; ?></tbody></table></div></details>
